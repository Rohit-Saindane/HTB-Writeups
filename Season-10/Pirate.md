...
```text
[*] When you coerce WEB01$ to authenticate, it sends an SMB-based NTLM authentication. But you want to relay that to LDAPS on the Domain Controller.
	 Here's the conflict:
...
[*] When you relay this to LDAPS without modification, the LDAPS server sees these flags and says: "Oh, this client wants signed communication. I'll enforce signing." But here's the catch – as the relay attacker, you don't have the session keys to actually sign the LDAPS traffic

The LDAPS connection fails because you can't meet the signing requirement that the original SMB authentication requested.
```
...
```text
1. Removes the MIC (Tamper-Proof Seal)
The MIC normally prevents any modification to the authentication message. By removing it, you can now modify the NTLM message without invalidating it .

2. Clears the "Require Signing" Flags
Once the MIC is gone, you can flip those three signing flags from "ON" to "OFF":
...
```text
roxychains evil-winrm -i WEB01.pirate.htb -u 'administrator' -H b1aac1584c2ea8ed0a9429684e4fc3e5
[proxychains] config file found: /etc/proxychains4.conf
[proxychains] preloading /usr/lib/x86_64-linux-gnu/libproxychains.so.4
[proxychains] DLL init: proxychains-ng 4.17
...
- 🔍 *As we know IT-Pirate has WirteSPN privilege over DC01, so we can add this new SPN.*
...
2.he Request: It says: "Look, I have this Service Ticket for the Administrator. I want to use his permissions to talk to the HTTP service on WEB01.pirate.htb."
