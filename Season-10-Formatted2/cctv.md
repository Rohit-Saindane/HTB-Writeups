...
> [!CAUTION]
> **CVE-2024-51428 - Authenticated SQL Injection in ZoneMinder**
> ZoneMinder v1.37.63 suffers from a time-based blind SQL injection vulnerability in the `tid` parameter within the `removetag` action. This allows an attacker to dump database hashes and sensitive configurations.
> **Mitigation:** Update ZoneMinder to the latest stable version that patches this SQL injection flaw. Validate and use prepared statements (parameterized queries) for all user-supplied inputs, especially the `tid` parameter. Ensure database users operate with the least privilege necessary.

> [!WARNING]
> **Default Credentials & Credential Stuffing**
> The ZoneMinder web portal was accessible using default/weak credentials (`admin:admin`).
> **Mitigation:** Disable default accounts upon installation or enforce a strong password policy. Additionally, implement multi-factor authentication (MFA) and rate-limiting on all login endpoints to prevent brute-forcing and credential stuffing.

> [!CAUTION]
> **CVE-2025-60787 - OS Command Injection in MotionEye**
> MotionEye v0.43.1b4 and earlier are vulnerable to command injection via configuration parameters such as `image_file_name`. Additionally, the service was running as `root`.
> **Mitigation:** Upgrade MotionEye to a secure version. Perform strict server-side validation on all configuration parameters to block dangerous shell metacharacters. Relying solely on client-side JavaScript validation (e.g., `configUiValid()`) is completely ineffective as it can be trivially bypassed. Furthermore, the MotionEye service should drop privileges and run as a dedicated, low-privileged user account instead of `root`.
